...
const downloadProtection = async (req, res, next) => {
	try {
		...
		const mode = (req.query.mode || 'view').toLowerCase(); // 'view' | 'download'
		...
		const post = await prisma.posts.findFirst({
			where: { image_url: { contains: imageUrl } }
		});
		...
		// Allow inline view, restrict actual download
		if (post.download_protected && mode === 'download') {
...
module.exports = { downloadProtection, preventRightClick };