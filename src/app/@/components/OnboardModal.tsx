// File: components/OnboardModal.tsx
...
export default function OnboardModal({ user }: { user: any }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white">
      <div className="w-full max-w-5xl px-4 py-8 md:px-10 md:py-12">
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex justify-center gap-6 flex-wrap">
                {["male", "female", "other"].map((value) => (
                  <button
                    key={value}
                    className={`px-8 py-4 rounded-full text-lg font-medium border shadow-md transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-black ${
                      gender === value ? "bg-black text-white" : "bg-white text-black hover:bg-gray-100"
                    }`}
              ...
              <button
                disabled={!gender}
                className="mt-10 px-10 py-4 rounded-full bg-black text-white text-lg font-semibold disabled:opacity-40 hover:bg-gray-800"
                onClick={() => setStep(2)}
              >
                Continue
              </button>
          ...
          {step === 2 && (
            ...
            >
              ...
              <p className="text-gray-500 text-md">Choose at least one to get started</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                {TOPICS.map((topic) => (
                  <button
                    key={topic.label}
                    onClick={() => handleTopicToggle(topic.label)}
                    className={`group relative overflow-hidden rounded-3xl shadow-md ring-2 transition-all duration-300 ${
                      topics.includes(topic.label) ? "ring-black" : "ring-transparent"
                    }`}
                  >
                    <img
                      src={topic.image}
              ...
              <button
                ...
                className="mt-6 px-8 py-3 rounded-full bg-black text-white text-lg disabled:opacity-40 hover:bg-gray-800"
                onClick={() => setStep(3)}
              >
                Continue
              </button>
          ...
          {step === 3 && (
            ...
            >
              ...
              <motion.button
                ...
                className="px-10 py-4 rounded-full bg-black text-white text-lg font-semibold hover:bg-gray-800 shadow-lg hover:shadow-xl transition-all duration-300"
